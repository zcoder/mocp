...
static int ask_for_tags (const struct plist *plist, const int tags_sel)
{
	...
	if (tags_sel != 0) 
    {
		for (i = 0; i < plist->num; i++) 
        {
			if (!plist_deleted(plist, i) && (!plist->items[i].tags || ~plist->items[i].tags->filled & tags_sel) && !is_cue(plist, i))
            {
				char *file;
...
static void update_curr_file ()
{
    ...
    if (curr_type == F_CUE_TRACK) 
    {
        ...
        stime = get_curr_stime ();
        etime = get_curr_etime ();
    }
    ...
    {
        if (sname[0] &&	(!curr_file.file || strcmp(sname, curr_file.file))) 
        {
            ...
            else
            {
                ...
                {
                    curr_file.title = files_iconv_str (strrchr(sname, '/') + 1);
                }
                else
                {
                    curr_file.title = xstrdup (strrchr(sname, '/') + 1);
                }
            }
            ...
            silent_seek_pos = -1;
            iface_set_curr_time (curr_file.curr_time);
            curr_file.total_time = (curr_type == F_CUE_TRACK ? (int) (etime - stime) : -1);
            iface_set_total_time (curr_file.total_time);
...
static void event_plist_add (const struct plist_item *item)
{
	if (plist_find_fname(playlist, item->type == F_CUE_TRACK ? item->title_tags : item->file) == -1)
    {
        ...
        {
		    if (options_get_int("ReadTags") && (!item->tags || !item->tags->title))
            {
		    	needed_tags |= TAGS_COMMENTS;
            }
		    if (!strcasecmp(options_get_str("ShowTime"), "yes") && (!item->tags || item->tags->time == -1))
            {
		    	needed_tags |= TAGS_TIME;
            }           
        ...
        {
            make_file_title (playlist, item_num, options_get_int ("HideFileExtension"));
        }
...
static void event_plist_del (char *file)
{
    ...
    {
		logit ("Server requested deleting an item not present on the playlist.");
    }
}
...
static void remove_dead_entries_plist ()
{
	...
	send_int_to_srv (CMD_LOCK);
	for (i = 0, file = plist_get_next_dead_entry(playlist, &i); file != NULL; file = plist_get_next_dead_entry(playlist, &i))
    {
...
void interface_cmdline_file_info (const int server_sock)
{
	...
	else 
    {
        ...
        {
            if (curr_file.state == STATE_PAUSE)
            {
			   puts ("State: PAUSE");
            }
        }
        ...
        else
        {
            if (curr_file.file[0]) 
            {
                ...
                {
				    curr_file.tags = get_tags_no_iface ( curr_file.file, TAGS_COMMENTS | TAGS_TIME);
                }
			    /* get the title */
			    if (curr_file.tags->title)
                {
		...
		const char *format_str)
{
	...
	str = str_repl(str, "%file", str_info.file ? str_info.file : "");
	str = str_repl(str, "%title", str_info.title ? str_info.title : "");
	str = str_repl(str, "%artist", str_info.artist ? str_info.artist : "");
	str = str_repl(str, "%song", str_info.song ? str_info.song : "");
	str = str_repl(str, "%album", str_info.album ? str_info.album : "");
	str = str_repl(str, "%tt", str_info.totaltime ? str_info.totaltime : "");
	str = str_repl(str, "%tl", str_info.timeleft ? str_info.timeleft : "");
	str = str_repl(str, "%ts", str_info.totalsec ? str_info.totalsec : "");
	str = str_repl(str, "%ct", str_info.currenttime ? str_info.currenttime : "");
	str = str_repl(str, "%cs", str_info.currentsec ? str_info.currentsec : "");
	str = str_repl(str, "%b", str_info.bitrate ? str_info.bitrate : "");
	str = str_repl(str, "%r", str_info.rate ? str_info.rate : "");
	str = str_repl(str, "\\n", "\n");
